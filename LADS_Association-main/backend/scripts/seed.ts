...
const localized = (en: string, fr: string, ar: string) => ({ en, fr, ar });
...
const LadsInfoSchema = new Schema(
  {
    title: { type: localizedField, _id: false },
    content: { type: localizedField, _id: false },
...
const LadsInfo = mongoose.model('LadsInfo', LadsInfoSchema);
...
async function seed() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    events.push({
      title: localized(
        `${cat} #${i + 1}: Building Community`,
        `${cat} #${i + 1}: Construire la communauté`,
        `${cat} #${i + 1}: بناء المجتمع`,
      ),
      description: localized(
        `Join us for our ${cat.toLowerCase()} event dedicated to ${pick(departments, i).toLowerCase()}. A unique opportunity to learn, share and connect with other members.`,
        `Rejoignez-nous pour notre événement ${cat.toLowerCase()} dédié à ${pick(departments, i).toLowerCase()}.`,
        `انضم إلينا في فعالية ${cat} مخصصة لـ ${pick(departments, i)}.`,
      ),
      category: cat,
  ...
  for (let i = 0; i < 25; i++) {
    ...
    activities.push({
      title: localized(
        `Activity ${i + 1}: ${cat} Program`,
        `Activité ${i + 1}: Programme de ${cat}`,
        `النشاط ${i + 1}: برنامج ${cat}`,
      ),
      description: localized(
        `Activity description ${i + 1}: a comprehensive ${cat.toLowerCase()} initiative aimed at improving local conditions.`,
        `Description de l'activité ${i + 1}: une initiative complète de ${cat.toLowerCase()}.`,
        `وصف النشاط ${i + 1}: مبادرة شاملة لـ ${cat}.`,
      ),
  ...
  for (let i = 0; i < 25; i++) {
    news.push({
      title: localized(
        `News article ${i + 1}: ${pick(newsTags, i)}`,
        `Article ${i + 1}: ${pick(newsTags, i)}`,
        `خبر ${i + 1}: ${pick(newsTags, i)}`,
      ),
      content: localized(
        `Full content of news article ${i + 1}. This article covers recent developments and updates from the LADS Association regarding ${pick(departments, i).toLowerCase()}.`,
        `Contenu complet de l'article ${i + 1}. Cet article couvre les développements récents.`,
        `المحتوى الكامل للخبر ${i + 1}. يغطي هذا المقال آخر التطورات.`,
      ),
  ...
  const infos = infoTitles.map((title, i) => ({
    ...
    content: localized(
      `Detailed content for "${title}". This page describes the organization's approach to ${title.toLowerCase()}.`,
      `Contenu détaillé pour "${title}". Cette page décrit l'approche de l'organisation.`,
      `محتوى مفصل لـ "${title}". تصف هذه الصفحة نهج المنظمة.`,
    ),
    updatedBy: adminId,
  }));
  ...
  for (let i = 0; i < 25; i++) {
    ...
    formations.push({
      ...
      description: localized(
        `Hands-on ${cat.toLowerCase()} training session ${i + 1}. Open to all members. Covers fundamentals and practical exercises.`,
        `Session de formation ${cat.toLowerCase()} pratique ${i + 1}. Ouverte à tous les membres.`,
        `جلسة تدريبية عملية ${cat} رقم ${i + 1}. مفتوحة لجميع الأعضاء.`,
      ),
